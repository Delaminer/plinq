...
setupLink("goto-reminders", "follow-up.html");
setupLink("goto-networks", "my-networks.html");
...
const handleEntry = (entry) => {
  chrome.tabs.create(
    ...
    (tab) => {
      console.log(
        `Loaded tab for ${entry.name} with index ${tab.id}/${tab.status}`
      );
    }
  );
};
...
const handleLinkedInPageUser = () => {
  function extractContactInfo(doc) {
    const mailLink = doc.querySelector(
...
const handleLinkedInPageList = () => {
  ...
  const entries = links
    .map((link) => ({
      ...
      occupation: link.querySelector(".mn-connection-card__occupation")
        .innerText,
      link: link.href,
    }))
    .slice(0, 1);
...
class LinkedInApi {
  ...
  async fetchAllConnections() {
    return await this.#fetchWithCsrf(
      "https://www.linkedin.com/voyager/api/relationships/dash/connections?decorationId=com.linkedin.voyager.dash.deco.web.mynetwork.ConnectionListWithProfile-15&count=1000&q=search&sortType=RECENTLY_ADDED&start=0"
    );
  }

  async fetchContactInfo(userId) {
    return await this.#fetchWithCsrf(
      `https://www.linkedin.com/voyager/api/identity/profiles/${userId}/profileContactInfo`
    );
  }
}

function setupRequestIntercept(tabId) {
  const listener = chrome.webRequest.onSendHeaders.addListener(
    function (details) {
      const header = details.requestHeaders.filter(
        (i) => i.name === "csrf-token"
      )[0];
      if (header !== undefined) {
